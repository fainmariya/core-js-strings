...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const charCodeOfValue = str.charCodeAt(i);
    count += charCodeOfValue;
  }
  return count;
}
...
function formatTime(minutes, seconds) {
  ...
  const secString = String(seconds);
...
function countVowels(str) {
  const listOfVowels = ['a', 'e', 'i', 'o', 'u', 'y'];

  let count = 0;

  for (let i = 0; i < str.length; i += 1) {
    if (listOfVowels.includes(str[i].toLowerCase())) {
      count += 1;
    }
  }
...
function isPalindrome(str) {
  const normalizedStr = str.toLowerCase().replaceAll(/[^a-z0-9]/gi, '');
  const newStr = normalizedStr.split('').reverse().join('');
...
function reverseWords(str) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    newList.push(newStr[i].split('').reverse().join(''));
  }
  return newList.join(' ');
}
/**
...
function extractNameFromTemplate(value) {
  ...
  return `${newArray[1]} ${newArray[2].replace('!', '')}`;
}
/**
